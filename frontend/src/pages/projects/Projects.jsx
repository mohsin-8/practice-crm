...
import { Box, Button, Flex, IconButton, Input, InputGroup, InputRightElement, Spinner, Table, TableContainer, Tbody, Td, Text, Th, Thead, Tr, useDisclosure, useToast } from '@chakra-ui/react';
import { IoCloseOutline, IoPencilOutline, IoSearch, IoTrashOutline } from 'react-icons/io5';
import { FaEye } from 'react-icons/fa6';
import { DeleteProjectUsers } from '../../redux/projects/projectsAction';
            ...
            <Layout>
                ...
                <Box p={"1.5rem"} bgColor={"#ffffff"} borderRadius={"0.5rem"}>
                    ...
                    <Box p={4}>
                                            ...
                                            isProjects?.map((data) => (
                                                <Tr key={data?._id}>
                                                    ...
                                                    <Td fontSize={"14px"} fontWeight={400}>
                                                        <Flex alignItems={"center"}>
                                                            {data?.assignMembers?.map((member) => {
                                                                ...
                                                                return (
                                                                    <Box key={member?._id} position="relative" zIndex={0} w={"30px"} h={"30px"} margin="0px -7px" _hover={{ pos: "relative", zIndex: "1", '.closeIcon': { display: 'flex', justifyContent: "center", alignItems: "center" } }}>
                                                                        ...
                                                                        <IconButton onClick={() => RemoveUser(data?._id, member?._id)} className="closeIcon" icon={<IoCloseOutline size={16} />} size={"s"} colorScheme="red" position="absolute" top="-5px" right="-2px" display="none" aria-label="Remove" borderRadius={"100px"} />
                                                                    </Box>
                                                                );
                                                            })}
                                                        </Flex>
                                                    ...
                                                    <Td>
                                                        ...
                                                        <IconButton onClick={() => { setSelectedProjectId(data?._id); onOpen(); }} variant="ghost" icon={<IoTrashOutline />} aria-label="Delete" color="red.500" size="md" />
                                                    </Td>
                                                </Tr>
                                            ))
                                        ) : (
                        ...
                        <DeleteProjectModal isOpen={isOpen} onClose={onClose} projectId={selectedProjectId} />
                        <EditProjectModal isOpen={isEditModalOpen} onClose={handleCloseEditModal} projectId={selectedProjectId} refreshUpdateTableData={refreshUpdateTableData} />
